...
void            main()
{
    ...
    //                                     N/S   U/D   E/W
    vec4 NEU = proj * view * model * vec4( len,  len,  len, 0.0);
    vec4 NED = proj * view * model * vec4( len, -len,  len, 0.0);
    vec4 NWU = proj * view * model * vec4( len,  len, -len, 0.0);
    vec4 NWD = proj * view * model * vec4( len, -len, -len, 0.0);
    vec4 SEU = proj * view * model * vec4(-len,  len,  len, 0.0);
    vec4 SED = proj * view * model * vec4(-len, -len,  len, 0.0);
    vec4 SWU = proj * view * model * vec4(-len,  len, -len, 0.0);
    vec4 SWD = proj * view * model * vec4(-len, -len, -len, 0.0);
